...
logger = make_logger(name="signal-feed")


@receiver(post_save, sender=Feed)
def signal_set_feed_metadata(sender, instance, created, **kwargs):
    """
    Сигнал для создания метаданных RSS-ленты
    """
    if created:
        logger.debug(f"Получен сигнал для создания метаданных RSS-ленты {instance=}")
